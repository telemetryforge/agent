...
apply_patch() {
    ...
    # Use --batch and --forward: --batch prevents prompts, --forward skips already-applied hunks
    if patch -p1 --batch --forward --dry-run < "$patch_file" >/dev/null 2>&1; then
        if [[ "$DRY_RUN" == false ]]; then
            # Get git status before applying
            local files_before
            files_before=$(git status --porcelain 2>/dev/null)

            if patch -p1 --batch --forward < "$patch_file" >/dev/null 2>&1; then
                # Verify files actually changed
                local files_after
                files_after=$(git status --porcelain 2>/dev/null)
    ...
    if [[ "$applied" == false ]]; then
        if patch -p1 --fuzz=2 --batch --forward --dry-run < "$patch_file" >/dev/null 2>&1; then
            if [[ "$DRY_RUN" == false ]]; then
                # Get git status before applying
                local files_before
                files_before=$(git status --porcelain 2>/dev/null)

                if patch -p1 --fuzz=2 --batch --forward < "$patch_file" >/dev/null 2>&1; then
                    # Verify files actually changed
                    local files_after
                    files_after=$(git status --porcelain 2>/dev/null)
